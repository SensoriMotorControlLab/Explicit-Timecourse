...
xgFeatures <- function () {
  ...
  mutate(
    ...
    # roll_median     = rollapply(aimdeviation_deg, width = 8, FUN = median, fill = NA, align = "right", partial = TRUE),
...
min_sd_trials <- strategy_annot %>%
  ...
  filter(!is.na(roll_sd) & trial_idx >= trial.start) %>% ##### make sure its sorted
  mutate(
    stable = abs(roll_sd - final_sd) <= delta,
    stable_run = rollapply(stable, width = N, FUN = all, fill = NA, align = "left") ###double check what FUN=all 
  ) %>%
  summarise(
    trial.end = case_when(
...
#FIND START FIRST WITH CG THEN TAKE OUTPUT OF THAT AND PUT IT IN SECOND XGBOOST MODEL   
