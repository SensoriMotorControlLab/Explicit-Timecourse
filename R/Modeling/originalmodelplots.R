...
total_learners_data <- total_learners_data %>%
  mutate(
    trial_rel = cutrial_no - ifelse(group == "Group 1", 89, 113)
  )
...
twoSt_data <- total_learners_data %>%
...
steps_all <- twoSt_data %>%
  group_by(participant_id) %>%
  filter(trial_type == "rotated") %>%
  mutate(trial_rel = cutrial_no - ifelse(group == "Group 1", 89, 113)) %>%
  summarise(
    baseline_value = aimdeviation_deg[trial_rel == 0][1],
    step1_trial = trial_rel[which(aimdeviation_deg - baseline_value > 10)[1]],
    step1_value = aimdeviation_deg[which(aimdeviation_deg - baseline_value > 10)[1]],
    step2_trial = trial_rel[which(aimdeviation_deg - step1_value > 7)[1]],
    step2_value = aimdeviation_deg[which(aimdeviation_deg - step1_value > 7)[1]],
    .groups = "drop"
  ) %>%
  mutate(
    plateau1_end = step1_trial + 20,
    plateau2_end = step2_trial + 50
  )


mean_traj <- twoSt_data %>%
  group_by(trial_rel) %>%
  summarise(
    mean_aim = mean(aimdeviation_deg, na.rm = TRUE),
    sd = sd(aimdeviation_deg, na.rm = TRUE),
...
exp_data <- total_learners_data %>%
  filter(participant_id %in% exp_participants)


baseline_exp <- exp_data %>%
  filter(trial_type == "aligned") %>%
  group_by(participant_id) %>%
  slice_tail(n = 10) %>%      
  mutate(trial_rel = row_number() - 11) %>%   
  ungroup()

rotated_exp <- exp_data %>%
  filter(trial_type == "rotated") %>%
  mutate(trial_rel = trial_rel)  

aligned_exp <- bind_rows(baseline_exp, rotated_exp) %>%
  filter(trial_rel >= -10, trial_rel <= 85)
