...
PCAPlot <- function() {
  ...
  set.seed(123)
  km_res <- kmeans(trial_pca[, 1:2], centers = 3, nstart = 25)
  trial_pca$cluster <- factor(km_res$cluster)
  

  p <- ggplot(trial_pca, aes(x = PC1, y = PC2, color = cluster, label = participant_id)) +
    geom_point(size = 3) +
    geom_text(vjust = -1, size = 3) +
    theme_minimal() +
    labs(title = "Cluster visualization on top 2 PCs",
         x = "PC1",
         y = "PC2")
  
  print(p)
  
  # Return useful objects in case needed later
  features_with_clusters <- cbind(extract$features_df, cluster = km_res$cluster)
...
learning_phase_table <- function() {
  ...
  step_table <- learning_phase %>%
    ...
    group_by(rotation) %>%
    summarise(
      percent_step_people = (n_distinct(participant_id) / total_participants) * 100,
    ...
    mutate(metric = recode(metric,
                           "percent_step_people" = "% Step people",
                           "mean_step_size" = "Mean step size",
                           "mean_step_trial" = "Mean step onset trial",
                           "mean_stable_aimdev"   = "Mean stable strategy size"))
  ...
  step_gt <- step_table %>%
    ...
    tab_header(
      title = "Table 1. Step Learners by Rotation"
    )
  ...
  explore_table <- learning_phase %>%
    ...
    group_by(rotation) %>%
    summarise(
      percent_explore_people = (n_distinct(participant_id) / total_participants) * 100,
  ...
  explore_gt <- explore_table %>%
    ...
    fmt_number(columns = everything(), decimals = 2) %>%
    tab_header(
      title = "Table 2. Erratic Learners by Rotation"
    )
  ...
  gradual_table <- learning_phase %>%
    ...
    group_by(rotation) %>%
    summarise(
      percent_gradual_people = (n_distinct(participant_id) / total_participants) * 100,
  ...
  gradual_gt <- gradual_table %>%
    gt(rowname_col = "metric") %>%
    fmt_number(columns = everything(), decimals = 2) %>%
    tab_header(title = "Table 3. Gradual Learners by Rotation")
...
allFinalBar <- function () { 
  ...
  ggplot(all_strategy_aim, aes(x = factor(rotation), y = stable_strategy_aimdev)) +
    geom_bar(data = mean_per_rotation, aes(y = mean_stable), 
             stat = "identity", fill = "skyblue", alpha = 0.5) +
    geom_jitter(width = 0.15, height = 0, aes(color = cluster), size = 2) +
    labs(x = "Rotation (deg)", y = "Final Strategy (Stable Aim)", 
